...
trait Units
{
     ...
     *
     * @param CarbonConverterInterface|Closure|DateInterval|string|Unit $unit
    ...
    public function addUnit(Unit|string $unit, $value = 1, ?bool $overflow = null): static
    {
        ...
        if ($unit === 'weekday') {
            ...
            $overflow === null && ($ucUnit = ucfirst($unit).'s') && ! ($this->{'local'.$ucUnit.'Overflow'} ?? static::{'shouldOverflow'.$ucUnit}())
        )))) {
            $day = $date->day;
        }
