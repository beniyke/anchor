...
class Option
{
    ...
    public static function tryFrom(ReflectionParameter $parameter): ?self
    {
        ...
        if ($self->typeName === 'bool' && $self->allowNull && in_array($self->default, [true, false], true)) {
        ...
        if ($self->allowNull && $self->default !== null) {
        ...
        } elseif ($self->typeName === 'array') {
            $self->mode = InputOption::VALUE_REQUIRED | InputOption::VALUE_IS_ARRAY;
        } else {
            $self->mode = InputOption::VALUE_REQUIRED;
        }

        if (is_array($self->suggestedValues) && ! is_callable($self->suggestedValues) && count($self->suggestedValues) === 2 && ($instance = $parameter->getDeclaringFunction()->getClosureThis()) && $instance::class === $self->suggestedValues[0] && is_callable([$instance, $self->suggestedValues[1]])) {
    ...
    public function resolveValue(InputInterface $input): mixed
    {
        ...
        if ($value === null && in_array($this->typeName, self::ALLOWED_UNION_TYPES, true)) {
            return true;
        }

        if ($this->typeName === 'array' && $this->allowNull && $value === []) {
            return null;
        }
