...
class Process implements IteratorAggregate
{
    ...
    public function waitUntil(callable $callback): bool
    {
        ...
        while (true) {
            ...
            foreach ($output as $type => $data) {
                if ($type !== 3) {
                    $ready = $callback($type === self::STDOUT ? self::OUT : self::ERR, $data) || $ready;
    ...
    public function getIterator(int $flags = 0): Generator
    {
        ...
        while ($this->callback !== null || ($yieldOut && ! feof($this->stdout)) || ($yieldErr && ! feof($this->stderr))) {
            if ($yieldOut) {
    ...
    public function getTermSignal(): int
    {
        ...
        if ($this->isSigchildEnabled() && $this->processInformation['termsig'] === -1) {
     ...
     *
     * @param bool|float|int|resource|string|Traversable|null $input The content
     *
     * @return $this
     *
    ...
    protected function buildCallback(?callable $callback = null): Closure
    {
        if ($this->outputDisabled) {
            return fn ($type, $data): bool => $callback !== null && $callback($type, $data);
        }
        ...
        return function ($type, $data) use ($callback, $out): bool {
            ...
            return $callback !== null && $callback($type, $data);
        };
    }
    ...
    private function close(): int
    {
        ...
        if ($this->exitcode === -1) {
            if ($this->processInformation['signaled'] && $this->processInformation['termsig'] > 0) {
    ...
    private function replacePlaceholders(string $commandline, array $env): string
    {
        return preg_replace_callback('/"\$\{:([_a-zA-Z]++[_a-zA-Z0-9]*+)\}"/', function ($matches) use ($commandline, $env) {
            if (! isset($env[$matches[1]]) || $env[$matches[1]] === false) {
