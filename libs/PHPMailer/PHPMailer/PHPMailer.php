...
class PHPMailer
{
    ...
    private function mailPassthru($to, $subject, $body, $header, $params)
    {
        ...
        if (! $this->UseSendmailOptions or $params === null) {
    ...
    public function setFrom($address, $name = '', $auto = true)
    {
        ...
        $pos = strrpos($address, '@');
        if ($pos === false or (! $this->has8bitChars(substr($address, ++$pos)) or ! static::idnSupported()) and ! static::validateAddress($address)) {
            $error_message = sprintf(
                '%s (From): %s',
                $this->lang('invalid_address'),
                $address
    ...
    public function punyencodeAddress($address)
    {
        ...
        $pos = strrpos($address, '@');
        if (static::idnSupported() and ! empty($this->CharSet) and $pos !== false
        ) {
    ...
    {
        if ($this->Mailer == 'smtp' or ($this->Mailer == 'mail' and stripos(PHP_OS, 'WIN') === 0)
        ) {
    ...
    {
        foreach ($this->attachment as $attachment) {
            if ($attachment[6] == 'inline' and $cid == $attachment[7]) {
                return true;
            }
        }
    ...
    public function DKIM_QP($txt)
    {
        ...
        for ($i = 0; $i < $len; $i++) {
            $ord = ord($txt[$i]);
            if ((($ord >= 0x21) and ($ord <= 0x3A)) or $ord == 0x3C or (($ord >= 0x3E) and ($ord <= 0x7E))) {
                $line .= $txt[$i];
            } else {
