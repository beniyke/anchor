...
class CreateWalletTransactionTable extends BaseMigration
{
    ...
    {
        Schema::createIfNotExists('wallet_transaction', function (SchemaBuilder $table) {
            $table->id();
            ...
            $table->indexIfNotExist(['wallet_id', 'created_at'], 'idx_wallet_transaction_created');
            $table->indexIfNotExist('reference_id', 'idx_wallet_transaction_reference');
            $table->indexIfNotExist('idempotency_key', 'idx_wallet_transaction_idempotency');
            $table->indexIfNotExist(['status', 'created_at'], 'idx_wallet_transaction_status');
            $table->indexIfNotExist('parent_transaction_id', 'idx_wallet_transaction_parent');

            $table->uniqueIfNotExist('reference_id', 'unique_wallet_transaction_reference');
            $table->uniqueIfNotExist('idempotency_key', 'unique_wallet_transaction_idempotency');
