...
class CreateWalletTable extends BaseMigration
{
    ...
    {
        Schema::createIfNotExists('wallet', function (SchemaBuilder $table) {
            $table->id();
            ...
            $table->indexIfNotExist(['owner_type', 'owner_id'], 'idx_wallet_owner');
            $table->indexIfNotExist('currency', 'idx_wallet_currency');
            $table->indexIfNotExist('updated_at', 'idx_wallet_updated');
            $table->uniqueIfNotExist(['owner_type', 'owner_id', 'currency'], 'unique_wallet_owner');
        });
    }
